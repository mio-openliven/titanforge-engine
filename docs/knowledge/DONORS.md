...
| `anvil-parser2` | <https://github.com/0xTiger/anvil-parser2> | MIT | Python reader/parser for Anvil region data with 1.18+ support, which is much closer to the 1.21.11 target than the archived original fork. | Good candidate for an isolated exporter-adapter spike that validates chunk/region assumptions without touching core generation. `TF-042` proves a first sampled `r.0.0.mca` write/read loop through `anvil-region-spike`, `TF-043` wraps that into a first sampled `save-shell` handoff, `TF-044` adds a minimal `test-world` candidate with `level.dat` and `session.lock`, and `TF-076` expands that bounded sample beyond one region file while still marking the result experimental. |
