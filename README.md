...
Use `first-map-status` when the pack already exists and you only need the current handoff summary again. It reads `first-map-manifest.json` and now reprints the preset intent, world-scale guidance, review order, next actions, Minecraft caution notes, and saved command hints without rebuilding anything. It also surfaces the current route preview and route-plan handoff plus ready-to-run region-focused and anchor-focused `first-map-test-world` commands, so a scenario writer can sample a named zone such as the harbor, coast, forest, village, or ridge, then narrow that further to anchors like `arrival`, `center`, `shoreline`, `ridge-vista`, or `approach`.
