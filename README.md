...
`anvil-test-world-status` now also tells you the sampled source origin, the next sampled window to try after a passed manual-open check, and on first-map wrapper runs it also tells you when to stop growth and go back to `first-map-status` after a failed manual sample.
