...
python -m titanforge first-map-refresh out\my-first-world
python -m titanforge first-map-resize out\my-first-world --width 8192 --length 6144
python -m titanforge first-map-retheme out\my-first-world --preset island-kingdom
python -m titanforge first-map-set-story out\my-first-world --premise "A sea-border kingdom where the capital hides what waits inland." --player-feeling "The player should feel welcome at first, then increasingly suspicious and small."
...
Use `first-map` when you want the fastest path from idea to the first reviewable map pack. It now writes a starter `titanforge.toml`, builds the first `project-location` output, writes a small root manifest, writes a short root `first-map-start.txt`, writes a root `review.html`, starts that page with a short do-this-next path for scenario writers, labels the requested world scale in plain language, surfaces the preset story intent and key regions, gives a few exact safe size examples with rerun commands, and tells you to open that root page first after the text handoff.

Use `first-map-status` when the pack already exists and you only need the current handoff summary again. It reads `first-map-manifest.json` and now reprints the preset intent, world-scale guidance, review order, next actions, Minecraft caution notes, and saved command hints without rebuilding anything. It starts with the root `first-map-start.txt` handoff, then a short recommended walkthrough built from the current route plan, surfaces one root `minecraft-first-pass.txt` path for the first in-world datapack pass, explains whether the starter sample stays in one `.mca` file or later grows into several sampled region files, then keeps the deeper route preview, route-plan, region-focused, and anchor-focused `first-map-test-world` commands available when a scenario writer needs to inspect a named zone such as the harbor, coast, forest, village, or ridge.

Use `first-map-refresh` after you manually edit `titanforge.toml` inside an existing `first-map` project. It rebuilds the current `first-map\...` outputs plus the root `review.html` and `first-map-manifest.json`, so the handoff stays honest instead of pointing at stale size or story data.

Use `first-map-resize` when you only need a different world footprint and do not want to edit TOML by hand. It updates `width` and `length` inside the existing `titanforge.toml`, then immediately runs the same refresh path so the root handoff and the generated `first-map\...` artifacts stay aligned.

Use `first-map-retheme` when the map should keep its current size but switch to another starter preset such as `coastal-valley`, `frontier-basin`, or `island-kingdom`. It replaces the current preset-backed premise, player feeling, and starter regions inside `titanforge.toml`, then refreshes the existing `first-map\...` outputs in one pass.

Use `first-map-set-story` when the preset foundation is still fine but the scenario text needs a different premise or player feeling. It rewrites the `[creative]` story fields inside `titanforge.toml`, then refreshes the existing `first-map\...` outputs in one pass.

Use `first-map-set-regions` when the preset is no longer enough and you want your own region lineup without hand-editing `[[regions]]` in TOML. Repeat `--region` for every zone you want to keep, using `title|kind|story role|mood|coverage[%][|notes]`, and TitanForge will rewrite the region block plus refresh the current `first-map\...` outputs in one pass.
...
Open `review.html` in a browser to inspect the first local TitanForge review page for that pack. It now starts with a short "how to read this pack" handoff, then shows coastline smoothing and terrain color drafts before the grayscale heightmap.
...
That same location review page now also starts with a short beginner review order, so a scenario writer can see what to inspect first before digging into raw files.
It now also shows the draft fixture command count and footprint inline, so a tester can judge scope without opening the JSON separately.
...
Use it when you want the quickest scenario-writer flow. Open the root `first-map-start.txt` first if you arrive from the folder view, then `review.html`; the root handoff now explains the preset story intent, the key starter regions, the logical world size, the plain-language world scale, the smaller draft raster, the `blocksPerPixel` bridge, and the sampled `.mca` file scope behind the recommended Minecraft starter shell before you jump into `first-map\location\review.html`, `first-map\draft\review.html`, or the Minecraft handoff files. The root `first-map-manifest.json` now also records that same guidance plus a machine-readable open order, next-action plan, command hints, Minecraft handoff artifact order, and a recommended sampled test-world size for future UI layers. If you revisit the folder later, `python -m titanforge first-map-status out\my-first-world` prints that handoff summary again without rerunning generation, `python -m titanforge first-map-refresh out\my-first-world` rebuilds the handoff after manual config edits, `python -m titanforge first-map-resize out\my-first-world --width 8192 --length 6144` changes size without hand-editing TOML, `python -m titanforge first-map-retheme out\my-first-world --preset island-kingdom` swaps the starter story/region lineup without hand-editing TOML, `python -m titanforge first-map-set-story out\my-first-world --premise "..." --player-feeling "..."` replaces the story text without hand-editing TOML, `python -m titanforge first-map-set-regions out\my-first-world --region "Harbor Town|city|arrival hub|busy, grounded|30%" ...` replaces the region lineup without hand-editing TOML, and `python -m titanforge first-map-test-world out\my-first-world` uses the stored safe starter sample for the shortest donor-backed path to a throwaway Minecraft manual-open candidate.
...
This writes one sampled `region\` folder with one or more `r.<x>.<z>.mca` files, a manifest, and a short README. It is intentionally clipped to a bounded chunk-aligned sampled window instead of pretending full world export is solved. It proves a donor-backed `.mca` write/read path without claiming that TitanForge already exports full playable worlds.
...
`anvil-test-world` is the next honest candidate: it writes `test-world\level.dat`, `test-world\session.lock`, and sampled `test-world\region\*.mca` files, but it still labels the result as an experimental throwaway manual-open shell rather than claiming verified world export.
