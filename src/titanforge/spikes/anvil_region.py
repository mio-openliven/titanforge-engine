...
def write_anvil_region_spike(
    ...
    *,
    ...
    sampled_width = min(config.width, max_side)
    sampled_length = min(config.length, max_side)
    sampled_cuboids = _clip_fixture_to_window(fixture.cuboids, sampled_width, sampled_length)
    if not sampled_cuboids:
        raise AnvilRegionSpikeError("The sampled window did not contain any block cuboids to export.")

    warnings: list[str] = []
    cropped = config.width > sampled_width or config.length > sampled_length
    if cropped:
        warnings.append(
            f"World {config.width} x {config.length} was clipped to the first {sampled_width} x {sampled_length} blocks "
    ...
    manifest = {
        ...
        "sampleWindow": {
            "origin": {"x": 0, "z": 0},
            "size": {"width": sampled_width, "length": sampled_length},
            "cropped": cropped,
        },
        "export": {
    ...
    readme_path.write_text("\n".join(_build_readme_lines(config, sampled_width, sampled_length, cropped, donor_version)) + "\n", encoding="utf-8")
...
def _build_readme_lines(
    ...
    crop_line = (
        f"The original {config.width} x {config.length} world was clipped to the first {sampled_width} x {sampled_length} blocks."
        if cropped
    ...
    return (
        ...
        f"- Sample window: {sampled_width} x {sampled_length} blocks from origin (0, 0)",
        f"- {crop_line}",
        "",
        "Limits:",
...
def _clip_fixture_to_window(
    ...
    for cuboid in cuboids:
        start_x = max(0, cuboid.x)
        start_z = max(0, cuboid.z)
        end_x = min(sampled_width, cuboid.x + cuboid.width)
        end_z = min(sampled_length, cuboid.z + cuboid.length)
