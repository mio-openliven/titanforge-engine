...
That terminal status is now meant to stand on its own: it repeats the preset intent, size guidance, review order, next change actions, and Minecraft-side cautions from `first-map-manifest.json`, not just raw file paths. It also includes region-focused and anchor-focused sample commands for the named story zones, so you can target places like the harbor, coast, forest, village, or ridge, then narrow further to exact anchors such as `arrival`, `center`, `shoreline`, or `ridge-vista`.
