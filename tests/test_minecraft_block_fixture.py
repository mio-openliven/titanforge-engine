...
class MinecraftBlockFixtureTests(unittest.TestCase):
    def test_build_block_fixture_supports_12111(self) -> None:
        ...
        self.assertTrue(fixture.supported)
        self.assertEqual(fixture.base_y, 64)
        self.assertGreater(len(fixture.cuboids), 0)
        source_types = {cuboid.source_type for cuboid in fixture.cuboids}
        self.assertIn("region-band", source_types)
        self.assertIn("road-strip", source_types)
        self.assertIn("settlement-pad", source_types)

    def test_build_block_fixture_marks_unsupported_targets(self) -> None:
        config = load_project_config(Path("examples/tiny_project/titanforge.toml"))
        world_plan = build_world_plan(config)
        transition_plan = build_transition_plan(world_plan)
        route_plan = build_route_plan(world_plan)
        ...
        fixture = build_minecraft_block_fixture("1.12.2", world_plan, transition_plan, road_plan, settlement_plan)

        self.assertFalse(fixture.supported)
        self.assertEqual(fixture.cuboids, ())
        self.assertIn("No block fixture adapter is implemented yet", fixture.notes[-1])

    def test_write_block_fixture_creates_json(self) -> None:
        config = load_project_config(Path("examples/tiny_project/titanforge.toml"))
        world_plan = build_world_plan(config)
        transition_plan = build_transition_plan(world_plan)
        route_plan = build_route_plan(world_plan)
        ...
        with tempfile.TemporaryDirectory() as directory:
            output_path = Path(directory) / "block-fixture.json"
            write_minecraft_block_fixture("1.21.11", world_plan, transition_plan, road_plan, settlement_plan, output_path)
